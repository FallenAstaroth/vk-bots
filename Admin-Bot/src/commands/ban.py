...
def ban_user(vk, db, peer_id, bot_id, from_id, split_text, data, admins, NOTICE):
    ...
    try:
        ...
        if from_id in admins:
            ...
            if user_id is None:
                ...
                if user_id != -bot_id and user_id not in admins:
                    ...
                    if NOTICE == 1: return send_msg(vk, peer_id, '❎ Невозможно забанить администратора', '')
        ...
        if NOTICE == 1: return send_msg(vk, peer_id, '❎ Не удалось забанить пользователя', '')
...
def unban_user(vk, db, peer_id, from_id, admins, split_text, data, NOTICE):
    ...
    try:
        ...
        if from_id in admins:
            ...
            if user_id is None:
                ...
                if db.check_user_banlist(user_id) is False:
                    ...
                    return send_msg(vk, peer_id, '❎ Пользователя нет в банлисте', '')

                else:

                    db.delete_user_banlist(user_id)

                    return send_msg(vk, peer_id, '✅ Пользователь разбанен', '')
        ...
        if NOTICE == 1: return send_msg(vk, peer_id, '❎ Не удалось разбанить пользователя', '')
